...
```text
//dev-static.devops.woa.com/<service>/
```
...
```text
auto switch:
  dev-static.devops.woa.com -> proxy
  其他域名 -> iOA
...
日常调试只需要选择 `auto switch`。不要直接选 `proxy`，否则所有非本机请求都会进 whistle，可能影响其他内网页面；也不要选 `iOA`，否则 `dev-static.devops.woa.com` 不会进入本地映射。
...
以 `devops-pipeline` 为例，正常情况下 `dev-static.devops.woa.com/pipeline/...` 下的资源应被映射到本地 `8006`。如果 iframe 中仍加载远端指纹资源，例如 `/pipeline/pipeline.<hash>.min.js` 来自远端环境，说明 Switchy Sharp 未切到 `auto switch`，或 whistle 中对应服务端口映射未启用。

### 4.3 whistle 规则和 Values

Switchy Sharp 只负责把 `dev-static.devops.woa.com` 转发给本机 whistle。真正把不同服务映射到本地端口的是 whistle 规则。
...
```text
dev-static.devops.woa.com/<service>/... -> http://127.0.0.1:<port>/<service>/...
```

例如调试 `devops-pipeline` 时，`dev-static.devops.woa.com/pipeline/...` 最终应落到本地 `8006` 的 `/pipeline/...` 资源；调试 `devops-nav` 时，主工程静态资源最终应落到本地 `8080`。
...
{
  "CORS": {
    "origin": "https://dev.devops.woa.com",
    "methods": "POST, GET",
    "headers": "x-csrftoken",
    "credentials": true,
    "maxAge": 300000
  ...
  "localNAV": {
    "origin": "https://local-devops.woa.com:8080",
    "methods": "POST, GET",
    "headers": "x-csrftoken",
    "credentials": true,
    "maxAge": 300000
...
`devops-nav` 当前已经配置了 `/ms -> https://dev.devops.woa.com`。其他工程调试前需要按实际请求方式检查，如果页面访问失败，优先看 Network 中 `/ms` 请求是否命中 `dev.devops.woa.com`。

## 5. HTTPS 与 `.woa.com` 本地域名

需要使用 `woa.com` 登录态的页面，本地调试域名必须以 `.woa.com` 结尾。不要使用 `localhost`、`127.0.0.1`、`local-stream.com` 作为需要登录态的调试入口。

推荐默认域名：

```text
local.devops.woa.com
```

特殊工程可使用更明确的域名，例如：

```text
local-ai.devops.woa.com
```

如果本机未解析域名，需要配置 hosts：

```text
127.0.0.1 local.devops.woa.com
127.0.0.1 local-ai.devops.woa.com
```
...
cd src/frontend
mkcert \
  -key-file local.devops.woa.com.pem \
  -cert-file local.devops.woa.com-crt.pem \
  local.devops.woa.com
```

仓库中已有 `src/frontend/local.devops.woa.com.pem` 和 `src/frontend/local.devops.woa.com-crt.pem`。如果本机浏览器不信任证书，重新执行 `mkcert -install` 并按本机环境重新生成证书。
...
devServer: {
    ...
    server: {
        type: 'https',
        options: {
            key: fs.readFileSync(path.join(__dirname, '../local.devops.woa.com.pem')),
            cert: fs.readFileSync(path.join(__dirname, '../local.devops.woa.com-crt.pem'))
        }
    }
}
```
...
export default {
    server: {
        host: 'local.devops.woa.com',
        https: {
            key: fs.readFileSync(path.resolve(__dirname, '../local.devops.woa.com.pem')),
            cert: fs.readFileSync(path.resolve(__dirname, '../local.devops.woa.com-crt.pem'))
        }
    }
}
```

如果某个工程仍使用非 `.woa.com` host，需要先改成本地 `.woa.com` 域名再调试依赖登录态的页面。
...
7. 如果服务需要接口，确认 dev server 已将 `/ms` 代理到 `https://dev.devops.woa.com`。
8. 如果页面依赖登录态，确认访问入口或映射后的页面仍处在 `.woa.com` 域名下。
9. 在 Chrome Network 中验证：
   - JS/CSS 等静态资源命中本地端口。
   - `/ms` 接口请求命中 `https://dev.devops.woa.com/ms`。
...
```text
验证页面：
https://dev.devops.woa.com/console/pipeline/_lockiechen_personal/list/template/allTemplate?mode=all
...
- Network 中静态资源命中本地端口。
- `/ms` 接口命中 `https://dev.devops.woa.com/ms`，没有登录态或跨域失败。
...
检查 dev server 的 proxy。静态资源能映射到本地不代表接口可用，`/ms` 仍需要回到 `https://dev.devops.woa.com`。

### 登录态丢失

确认页面访问域名以 `.woa.com` 结尾，并且 HTTPS 证书被 Chrome 信任。不要使用 `localhost` 或 `127.0.0.1` 验证依赖登录态的页面。
