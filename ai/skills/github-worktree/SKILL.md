...
description: >-
  ...
  在 master 上：确定关联的 GitHub Issue（默认关联已有，明确要建时才在上游建单）、生成分支名、git fetch、脚本创建 worktree、cursor --new-window 打开新窗口；细节与分支命名见正文。
...
脚本创建 worktree 成功后会自动从主仓库同步 `.idea` 和 `.cursor` 到新 worktree，保证
...
- **add**：创建 worktree（如分支不存在则从 `-B/-Base` 指定的 base 新建，默认 master）
- **list**：列出所有 worktree
